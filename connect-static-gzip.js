  ...
  fs.stat(path, function(err, stat){
    ...
    function streamRange() {
      ...
      if (ranges) {
        ...
        stream = fs.createReadStream(path, ranges[0]);
    ...
    function checkGzipped() {
      fs.stat(path + '.gz', function (err, newStat) {
        if (err) {
          return streamEntireFile();
        }
    ...
    // user would prefer gzipped data
    if (-1 !== (req.headers['accept-encoding']||'').indexOf('gzip')) {
      return checkGzipped();
    }
