    ...
    function handleLine(next, teststr) {
      ...
      fs.lstat(root + '/' + md5sum + ext, function (err, stat) {
        if (err) {
          ...
          console.error(root + '/' + md5sum + ext + ': ' + pathname + '/' + name);
          next();
          return;
        }
        ...
        mediabox.importStrategy.copyHelper(root, stat, next, betterFileStats);
        //console.log(root, stat, null, betterFileStats); next();
      })
    }
    ...
    fs.readFile(__dirname + '/../MediaBox-storage/logs/db-all.txt', 'utf8', function (err, data) {
      ...
      */
      forEachAsync(data.split(/\n/), handleLine)
        .then(mediabox.importStrategy.syncAndEnd)
        ;
    });
  }
