(function () {
  ...
  function create(config) {
    ...
    function getAudioMeta(req, res, next) {
      ...
      var metas = mediabox.caches.meta.getAll()
        , audioMap
        , result
        ;

      audioMap = mediabox.caches.audio.getAllMap();

      metas.forEach(function (meta) {
        audioMeta = audioMap[meta.md5];

        if (audioMeta) {
          audioMeta.paths = audioMeta.paths || [];
          audioMeta.push(meta.path);
        }
      });

      result = normalizeTags(mediabox.caches.audio.getAll());
