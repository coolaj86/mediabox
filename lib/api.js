...
(function () {
  ...
  function create(config) {
    ...
    function createFormaline(req, res, formCompleteCallback) {
      ...
      function checkFilestart(fileInfo, chunk) {
        if (
          ...
          || (!isNaN(lastFileInfo.value.size) && !isNaN(fileInfo.value.size) 
              && (lastFileInfo.value.size < fileInfo.value.size))
        ) {
          if (!chunk) {
    ...
    staticServer.use('/files', connect.static(config.filesroot + '/all'));
    staticServer.use('/files', connect.static(__dirname + '/../../MediaBox-storage/db'));
