(function () {
  ...
  function create(config) {
    ...
    function getAudioMeta(req, res, next) {
      ...
      var audioCache = mediabox.caches.audio
      ...
      if (audioLen && audioLen !== audioArr.length) {
        ...
        audioCache.lastModified = Date.now();
      }
      audioCache.eTag = audioCache.eTag || UUID.v4().replace('-', '').substr(0,24);
      audioCache.lastModified = audioCache.lastModified || Date.now();
      audioLen = audioArr.length;

      res.setHeader('ETag', audioCache.eTag);
      res.setHeader('Date', toRfc822String(new Date()));
      res.setHeader('Last-Modified', toRfc822String(new Date(Date.now() - (24 * 60 * 60 * 1000))));
      res.setHeader('Expires', toRfc822String(new Date(Date.now() + (180 * 24 * 60 * 60 * 1000))));
      if (!modified && req.headers['if-modified-since']) {
        res.setHeader('Age', parseInt((Date.now() - audioCache.lastModified) / 1000, 10));
        res.statusCode = 304;
        res.end();
        return;
      }
    ...
    staticServer.use('/files', connect.createServer(
        connect.static(config.filesroot)
