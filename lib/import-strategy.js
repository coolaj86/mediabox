...
(function () {
  ...
  function removeFile(cb, fileStats) {
    fs.unlink(fileStats.path + '/' + fileStats.name, function (err) {
      if (err) {
        // this can be ignored as it's most likely that the file
        // was already removed with `mv` or `link` and `unlink`
  ...
  function createImporter(options, realroot, rootDeviceId) {
    ...
    function copyHelper(root, fileStats, next, betterFileStats) {
      ...
      function copyIfRealpathIsInScope(realpath) {
        console.log('copyIfRealpathIsInScope');
        ...
        console.log('putting in the store');
        store.put(enqueForRemoval, realpath, fileStats, betterFileStats);
      }

      function resolveRealpath() {
        console.log('resolveRealPath');
